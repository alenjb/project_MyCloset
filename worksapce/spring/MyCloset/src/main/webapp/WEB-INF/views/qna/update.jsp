...
<script>
	//사진 파일을 바꿨을 때
	$(function(){document.getElementById('qPhotoFile').onchange = function(e){
		 readImage(e.target)}
	});
									...
									<form action="update" method="post"
										...
										<div class="card-body">
											<div class="row mb-3">
												...
												<textarea name="qTitle" id="title" class="form-control"
													rows="1">${vo.QTitle}</textarea>
											</div>
											<div class="row mb-3">
												<label class="col-sm-2 col-form-label" for="content">내용</label>
												<textarea name="qContent" id="content" class="form-control"
													rows="7">${vo.QContent}</textarea>
											</div>
											<div class="row mb-3">
												...
												<div
													...
													<div class="button-wrapper">
														<label for="qPhotoFile"
															...
															type="file" id="qPhotoFile"
															name="qPhotoFile" class="account-file-input"
															hidden
										...
										<input type="hidden" name="qWriter" value="<%=id%>">
										<input id="no" name="no" value="${vo.no}" hidden="hidden"/>
										<input id="aTitle" name="aTitle" value="${vo.ATitle}" hidden="hidden"/>
										<input id="aContent" name="aContent" value="${vo.AContent}" hidden="hidden"/>
										<input id="aWriter" name="aWriter" value="${vo.AWriter}" hidden="hidden"/>
										<input id="aDate" name="aDate" value="<fmt:formatDate value="${vo.ADate}" pattern="yyyy-MM-dd"/>" hidden="hidden"/>
										<input type="file" id="aPhotoFile" name="aPhotoFile" value="${vo.APhotoFile}" hidden="hidden"/>
